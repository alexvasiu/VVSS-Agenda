...
public class Activity {
	...
	public Activity(Integer idActivitate, Date start, Date end, String name, List<Contact> contacts,
                    String description) {
	...
	public static Activity fromString(String line, RepositoryContact repcon) {
		try {
			...
			Integer idActivitate = Integer.parseInt(str[0]);
			String name = str[1];
			Date start = new Date(Long.parseLong(str[2]));
			Date duration = new Date(Long.parseLong(str[3]));
			String description = str[4];
			List<Contact> conts = new LinkedList<Contact>();
			for (int i = 5; i < str.length; i++) {
				conts.add(repcon.getByName(str[i]));
			}
			return new Activity(idActivitate, start, duration, name, conts, description);
		} catch (Exception e) {
			return null;
		}
	}
